...
class ArchitectureKnowledgeRetriever:
    ...
    def load_from_csv(self, csv_path: str, id_col: str = None, content_col: str = 'content'):
        ...
        for idx, row in df.iterrows():
            ...
            # 提取元数据（如果有）
            metadata = {col: row[col] for col in df.columns if col not in [id_col, content_col]}
...
# 测试代码
def demo_architecture_knowledge_retriever(query: str):
    """演示RISC-V架构知识检索器的使用"""
    df1 = pd.read_csv("D:/PythonCodes/aiops_mcp/knowledge_base/risc_v_knowledge_base.csv")
